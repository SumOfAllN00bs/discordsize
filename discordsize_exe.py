...
def select_file():
    filename = filedialog.askopenfilename(filetypes=[("Video files", "*.mp4 *.avi *.mov *.mkv *.flv *.gif *.gifv *.webm *.ogv *.opus *.mpeg *.3gp *.wmv")])
...
ttk.Label(root, text="Input Video Path").grid(row=0, column=0, padx=5, pady=5)
videopath = ttk.Entry(root, width=50)
videopath.grid(row=0, column=1, padx=5, pady=5)
ttk.Button(root, text="Browse", command=select_file).grid(row=0, column=2, padx=5, pady=5)

ttk.Label(root, text="Force Audio Rate (0 = don't force, 32 64 128 good options)").grid(row=1, column=0, padx=5, pady=5)
audio_rate = ttk.Entry(root)
audio_rate.grid(row=1, column=1, padx=5, pady=5)
...
scale_check.grid(row=2, column=0, columnspan=2, padx=5, pady=5)

ttk.Label(root, text="Video Width").grid(row=3, column=0, padx=5, pady=5)
vidwidth = ttk.Entry(root, state="disabled")
vidwidth.grid(row=3, column=1, padx=5, pady=5)

ttk.Label(root, text="Video Height").grid(row=4, column=0, padx=5, pady=5)
vidheight = ttk.Entry(root, state="disabled")
vidheight.grid(row=4, column=1, padx=5, pady=5)
